...
class Triangle{
	...
	public:
  	...
  	void CalcBoundingBox( double* min_corner , double* max_corner );
		bool IntersectsLine( double* P0 , double* P1 , int* sign , double* inv_dir );
		...
		void CalculateAABB();
		double CalculateDistanceToPoint( double* point );
		bool ProjectPointToTriangle( double* point , double* new_point );
		double DistanceToNode( int i_node , double* coord );
		void Copynode( int i_node , double* coord );
		double DistanceToSide( int i_side , double* coord , double* possible_point );
};
...
class Boundary{
	...
	public:
		...
		void CalculateAABBForTriangles();

		//SAVING ON FILE
		void SaveBoundaryMeshOnTxtFile( char* name );
